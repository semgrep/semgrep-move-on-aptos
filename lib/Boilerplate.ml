...
let map_script_user_decl (env : env) ((v1, v2) : CST.script_user_decl) =
  let v1 =
    (match v1 with
    | Some x -> R.Option (Some (
...
and map_anon_choice_expr_ae0e30e (env : env) (x : CST.anon_choice_expr_ae0e30e) =
  (match x with
  | `Expr x -> R.Case ("Expr",
      map_expr env x
    )
...
let map_script_spec_block (env : env) ((v1, v2) : CST.script_spec_block) =
  let v1 =
    (match v1 with
    | Some x -> R.Option (Some (
        map_attributes env x
...
let map_function_decl (env : env) ((v1, v2, v3, v4, v5, v6, v7, v8) : CST.function_decl) =
  let v1 =
    (match v1 with
    | Some tok -> R.Option (Some (
  ...
  let v7 =
    ...
    | None -> R.Option None)
  in
  let v8 = map_anon_choice_blk_f78fea4 env v8 in
  R.Tuple [v1; v2; v3; v4; v5; v6; v7; v8]

let map_script_constant_decl (env : env) ((v1, v2) : CST.script_constant_decl) =
  let v1 =
    (match v1 with
    | Some x -> R.Option (Some (
        map_attributes env x
...
let map_script_func_decl (env : env) ((v1, v2, v3) : CST.script_func_decl) =
  let v1 =
    (match v1 with
    | Some x -> R.Option (Some (
        map_attributes env x
...
let map_script (env : env) ((v1, v2, v3, v4, v5, v6, v7) : CST.script) =
  ...
  let v2 = (* "{" *) token env v2 in
  let v3 = R.List (List.map (map_script_user_decl env) v3) in
  let v4 =
...
let map_source_file (env : env) (x : CST.source_file) =
  ...
  | `Semg_exp (v1, v2) -> R.Case ("Semg_exp",
      ...
      let v2 = map_anon_choice_expr_ae0e30e env v2 in
      R.Tuple [v1; v2]
    )
